...
6. **`_id` is `video.<provider>-<sanitised id>`.** Section 9: "keyed by an id
   ...
   **Identity is the parsed video, not the URL string.** `youtu.be/<id>`,
   `youtube.com/watch?v=<id>` and `/embed/<id>` are one video and must ingest
   once — but a lesson is joined to its video on the URL *it* stores, so the
   spellings that lost the tie cannot simply be dropped or those lessons would
   match no video document at all. So the fetch step groups source URLs by
   `docId` and keeps every spelling, the document carries them in `urls` (with
   `url` first), and both the join in `VIDEO_MOMENTS_QUERY` and the one in the
   agent's instructions match `url` or `urls`. The build fails if a spelling
   does not round-trip to its own document id, or if two documents claim one.
...
- Every invocation goes through `spawnSync` with `shell: true`, a per-video
  timeout, and stderr captured so a failure names the video. `shell: true` is
  forced by Windows — `yt-dlp` installs as a `.cmd` shim, which `spawn` cannot
  execute directly — and it means arguments are concatenated into a command
  line rather than escaped. `cmd.exe` has no escape for a quote inside a quoted
  argument, so quoting alone cannot make an arbitrary value safe: the guarantee
  comes from `lib/video-id.mjs`, which pins each provider's id shape
  (`[A-Za-z0-9_-]{11}` for YouTube, digits + guid for Bunny) so a URL reaching
  the shell cannot carry a quote or a metacharacter. Everything else passed is
  ours — the output path and fixed literals like `en.*,en`.
